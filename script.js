...
async function cerrarSesion() {
  ...
  // Resetear el botón de login (quedaba en "Procesando..." desde el último ingreso)
...
async function cambiarPassword(btn) {
  const actual = document.getElementById('passActual').value;
  const nueva  = document.getElementById('passNueva').value;
  const nueva2 = document.getElementById('passNueva2').value;

  if (!actual || !nueva || !nueva2) return showToast('Completá todos los campos', 'warning', 'warning');
  if (nueva.length < 6)  return showToast('La nueva contraseña debe tener al menos 6 caracteres', 'warning', 'warning');
  if (nueva !== nueva2)  return showToast('Las contraseñas nuevas no coinciden', 'warning', 'warning');
  if (nueva === actual)  return showToast('La nueva debe ser distinta a la actual', 'warning', 'warning');
  if (!currentUser?.email) return showToast('No se pudo identificar tu usuario', 'warning', 'warning');
  ...
  try {
    // 1) Verificar la contraseña actual con un cliente temporal (no toca tu sesión)
    const tempClient = window.supabase.createClient(SUPABASE_URL, SUPABASE_ANON, {
      auth: { persistSession: false, autoRefreshToken: false, detectSessionInUrl: false }
    });
    const { error: errLogin } = await tempClient.auth.signInWithPassword({
      email: currentUser.email,
      password: actual
    });
    if (errLogin) {
      showToast('La contraseña actual es incorrecta', 'warning', 'warning');
      return;
    }

    // 2) Actualizar a la nueva contraseña (sobre tu sesión activa)
    const { error: errUpdate } = await window.sb.auth.updateUser({ password: nueva });
    if (errUpdate) throw errUpdate;

    document.getElementById('passActual').value = '';
    document.getElementById('passNueva').value  = '';
    document.getElementById('passNueva2').value = '';
    showToast('Contraseña actualizada correctamente', 'success', 'check_circle');
  } catch (err) {
    showToast(err.message || 'No se pudo actualizar la contraseña', 'warning', 'warning');
  } finally {
    btn.disabled = false;
...
window.sb.auth.onAuthStateChange(async (event, session) => {
  // No recargar toda la app en updates de usuario ni en refresh de token
...
function toggleTheme(isDark) {
  ...
  // Ajusta el color de la barra del navegador en mobile
  const meta = document.querySelector('meta[name="theme-color"]');
  if (meta) meta.setAttribute('content', isDark ? '#0E0E16' : '#1A1A2E');
}

function syncThemeToggle() {
  const cb = document.getElementById('modoOscuro');
...
function toggleProgresoDetalle() {
  document.getElementById('progresoDetalle').classList.toggle('open');
}
...
function switchTab(tab, btn) {
  ...
  aplicarModoTexto(tab === 'text');
}

// ── MODO TEXTO = carga manual (sin IA) ──
function aplicarModoTexto(activo) {
  const panelText = document.getElementById('panel-text');
  const toggle    = document.getElementById('modoDetalleToggle');
  const btnProc   = document.getElementById('btnAnalyze');
  const titulo    = document.getElementById('resultTitleText');

  // Ocultar textarea, hint, switch de detalle y botón Procesar
  if (panelText) panelText.style.display = activo ? 'none' : '';
  if (toggle)    toggle.style.display    = activo ? 'none' : '';
  if (btnProc)   btnProc.style.display   = activo ? 'none' : '';
  if (titulo)    titulo.textContent = activo ? 'Cargá el gasto' : 'Revisá y confirmá los gastos';

  if (activo) showMultiResult([{}]); // formulario vacío listo para completar
}

// Lee lo que hay cargado en el formulario (para no perderlo al agregar un ítem)
function leerItemsDelForm() {
  return Array.from(document.querySelectorAll('.result-item')).map(el => {
    const i = el.id.replace('item-', '');
    return {
      fecha:       document.getElementById(`i${i}-fecha`)?.value || '',
      monto:       document.getElementById(`i${i}-monto`)?.value || '',
      categoria:   document.getElementById(`i${i}-categoria`)?.value || '',
      medio:       document.getElementById(`i${i}-medio`)?.value || '',
...
function agregarItemManual() {
  showMultiResult([...leerItemsDelForm(), {}]);
}
...
// Devuelve { mesKey, label } del mes anterior al que se está viendo
function getMesAnteriorInfo() {
  let m = mesActual - 1;
  let a = anioActual;
  if (m < 0) { m = 11; a--; }
  return {
    mesKey: `${a}-${String(m + 1).padStart(2, '0')}`,
    label: `${MESES_ES[m]} ${a}`
  };
}

async function copiarMesAnterior() {
  const destinoKey = getMesKey();
  const { mesKey: origenKey, label: origenLabel } = getMesAnteriorInfo();

  // 1) Cargar el mes origen si no está en cache
  if (!presupuestoData[origenKey]) await loadPresupuestoForMes(origenKey);
  const origen = presupuestoData[origenKey] || { ingresos: [], egresos: [] };

  const items = [
    ...origen.ingresos.map(i => ({ tipo: 'ingreso', concepto: i.concepto, monto: i.monto })),
    ...origen.egresos.map(e => ({ tipo: 'egreso', concepto: e.concepto, monto: e.monto })),
  ];
  if (!items.length) {
    return showToast(`${origenLabel} no tiene presupuesto cargado`, 'warning', 'warning');
  }

  // 2) Confirmar si el mes destino ya tiene algo cargado
  const destino = presupuestoData[destinoKey] || { ingresos: [], egresos: [] };
  const yaTiene = destino.ingresos.length + destino.egresos.length;
  if (yaTiene && !confirm(
    `Este mes ya tiene ${yaTiene} registro${yaTiene !== 1 ? 's' : ''}. ` +
    `Se van a AGREGAR ${items.length} de ${origenLabel}. ¿Continuar?`
  )) return;
  ...
  const { data, error } = await window.sb.from('presupuesto_items').insert(rows).select();
  if (error) return showToast('Error al copiar: ' + error.message, 'warning', 'warning');

  // 4) Actualizar cache local
  if (!presupuestoData[destinoKey]) presupuestoData[destinoKey] = { ingresos: [], egresos: [] };
  data.forEach(it => {
    const item = { id: it.id, concepto: it.concepto, monto: parseFloat(it.monto) || 0 };
    (it.tipo === 'ingreso' ? presupuestoData[destinoKey].ingresos : presupuestoData[destinoKey].egresos).push(item);
  });

  renderPresupuesto();
  renderProgresoInicio();
  showToast(`${data.length} registros copiados de ${origenLabel}`, 'success', 'check_circle');
}
...
async function renderPresupuesto() {
  ...
  // Botón "Copiar mes anterior": solo si este mes está vacío
  const wrap = document.getElementById('presuCopiarWrap');
  if (wrap) {
    const vacio = data.ingresos.length === 0 && data.egresos.length === 0;
    wrap.style.display = vacio ? 'block' : 'none';
    if (vacio) {
      document.getElementById('btnCopiarLabel').textContent = `Copiar de ${getMesAnteriorInfo().label}`;
    }
  }
...
// Mes que se está viendo en "Progreso del presupuesto" (0 = mes en curso, -1 = anterior, ...)
let progresoOffset = 0;

function getProgresoMesInfo() {
  const d = new Date();
  d.setDate(1); // evita el bug de saltar meses desde el día 29/30/31
  d.setMonth(d.getMonth() + progresoOffset);
  const mes = d.getMonth(), anio = d.getFullYear();
  return {
    mes, anio,
    mesKey: `${anio}-${String(mes + 1).padStart(2, '0')}`,
    label: `${MESES_ES[mes]} ${anio}`
  };
}

function cambiarMesProgreso(delta) {
  const nuevo = progresoOffset + delta;
  if (nuevo > 0) return; // no dejamos ir al futuro
  progresoOffset = nuevo;
  renderProgresoInicio();
}
...
async function renderProgresoInicio() {
  const info = getProgresoMesInfo();
  const mesKey = info.mesKey;
  const lbl = document.getElementById('progresoMes');
  if (lbl) lbl.textContent = `${info.label} · gastado vs. presupuestado`;

  // Deshabilitar la flecha derecha cuando ya estamos en el mes en curso
  const btnNext = document.getElementById('btnProgresoNext');
  if (btnNext) {
    const enCurso = progresoOffset >= 0;
    btnNext.disabled = enCurso;
    btnNext.style.opacity = enCurso ? '0.25' : '1';
    btnNext.style.cursor = enCurso ? 'default' : 'pointer';
  }

  if (!presupuestoData[mesKey]) await loadPresupuestoForMes(mesKey);
  const egresos = (presupuestoData[mesKey]?.egresos) || [];

  const general = document.getElementById('progresoGeneral');
  const detalle = document.getElementById('progresoDetalle');
  const cont = document.getElementById('progresoList');
  if (!cont) return;

  // Sin egresos presupuestados
  if (!egresos.length) {
    if (general) general.innerHTML = `<div style="text-align:center; padding:16px; color:var(--text-3); font-size:13px;">
  ...
  expenses.forEach(e => {
    const d = new Date(e.fecha + 'T12:00:00');
    if (d.getMonth() === info.mes && d.getFullYear() === info.anio) {
      realPorCat[e.categoria] = (realPorCat[e.categoria] || 0) + e.monto;
    }
  });

  // ── Barra general (total del mes) ──
  const totalPresu = egresos.reduce((s, eg) => s + (parseFloat(eg.monto) || 0), 0);
  const totalReal  = egresos.reduce((s, eg) => s + (realPorCat[eg.concepto] || 0), 0);
  const genPct = totalPresu > 0 ? (totalReal / totalPresu) * 100 : (totalReal > 0 ? 100 : 0);
  const genBarW = Math.min(genPct, 100);
  let genCls = 'ok';
  if (totalPresu > 0 && totalReal > totalPresu) genCls = 'over';
  else if (genPct >= 80) genCls = 'warn';
  const genRight = totalReal > totalPresu
    ? `<span class="prog-over">+${formatMonto(totalReal - totalPresu)} excedido</span>`
    : `<span>Queda ${formatMonto(totalPresu - totalReal)}</span>`;
  general.innerHTML = `
    <div class="prog-general">
      <div class="prog-top">
        <span class="prog-general-label">Total del mes</span>
        <span class="prog-pct ${genCls}">${Math.round(genPct)}%</span>
      </div>
      <div class="prog-bar-wrap" style="height:14px;"><div class="prog-bar ${genCls}" style="width:${genBarW}%;"></div></div>
      <div class="prog-figs">
        <span>${formatMonto(totalReal)} de ${formatMonto(totalPresu)}</span>
        ${genRight}
      </div>
    </div>`;

  // ── Detalle por categoría (igual que antes) ──
...
// ── Auto-actualización al cambiar de mes ──
let _mesKeyVisto = currentMesKey();

function chequearCambioDeMes() {
  const hoy = currentMesKey();
  if (hoy === _mesKeyVisto) return;

  _mesKeyVisto = hoy;
  progresoOffset = 0;                       // volver al mes en curso
  mesActual = new Date().getMonth();        // sincronizar el selector de Presupuesto
  anioActual = new Date().getFullYear();

  renderProgresoInicio();
  rebuildFilterCat();
  showToast('Empezó un mes nuevo — presupuesto actualizado', 'info', 'event');
}

setInterval(chequearCambioDeMes, 60000);
document.addEventListener('visibilitychange', () => {
  if (!document.hidden) chequearCambioDeMes();
});
...
const _switchScreen = function(name) {
  ...
  if (name === 'config') syncThemeToggle();
...
function showMultiResult(items) {
  ...
  // En modo manual, botón para sumar otro gasto sin usar IA
  if (currentTab === 'text') {
    list.innerHTML += `
      <button class="btn-presu-add" onclick="agregarItemManual()" style="margin-top:4px;">
        <span class="material-icons-round">add</span> Agregar otro gasto
      </button>`;
  }
...
function removeItem(idx) {
  ...
  if (remaining === 0) {
    if (currentTab === 'text') showMultiResult([{}]);
    else hideResult();
  }
}
...
function discardResult() {
  ...
  if (currentTab === 'photo') {
    ...
    // En modo manual dejamos un formulario limpio listo para el próximo gasto
    showMultiResult([{}]);
  }
}
...
function renderTable() {
  ...
  // Solo los primeros TABLE_LIMIT (con el orden por defecto = los 10 más recientes)
  const visibles = verTodos ? filteredExpenses : filteredExpenses.slice(0, TABLE_LIMIT);

  if (footer) {
    if (filteredExpenses.length > TABLE_LIMIT) {
      footer.style.display = 'block';
      document.getElementById('tableFooterLabel').textContent =
        `Mostrando ${visibles.length} de ${filteredExpenses.length}`;
      document.getElementById('iconVerTodos').textContent = verTodos ? 'expand_less' : 'expand_more';
      document.getElementById('textVerTodos').textContent = verTodos ? 'Ver menos' : 'Ver todos';
    } else {
...
function updateStats() {
  ...
  // Divisor de días para el promedio:
  //  - si hay rango de fechas (desde/hasta) => días del rango
  //  - si no => días distintos en los que hubo gastos (comportamiento actual)
  const _period = document.getElementById('filterPeriod').value;
  const _from = document.getElementById('filterFrom').value;
  const _to = document.getElementById('filterTo').value;

  let diasUnicos;
  if (_period === 'custom' && _from && _to) {
    const msPorDia = 1000 * 60 * 60 * 24;
    const d1 = new Date(_from + 'T12:00:00');
    const d2 = new Date(_to + 'T12:00:00');
    diasUnicos = Math.round((d2 - d1) / msPorDia) + 1; // +1 para incluir ambos extremos
    if (diasUnicos < 1) diasUnicos = 1;
  } else {
    diasUnicos = new Set(filteredExpenses.map(e => e.fecha)).size;
  }
